...
def mover_objetos(evento, opcao):
...
def movimento_individual(evento):
    global pos_cubo, pos_triangulo, pos_piramide
...
def display_menu():
    ...
    print("6 - Controle individual de todos objetos")
    ...
    while rodando:
        for evento in pygame.event.get():
            if evento.type == QUIT:
                ...
                controle_global(evento)

                
                if opcao in ["1", "2", "3", "4", "5"]:
                    mover_objetos(evento, opcao)

                
                if opcao == "6":
                    movimento_individual(evento)
